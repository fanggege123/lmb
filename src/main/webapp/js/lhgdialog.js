...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	time: function( second, callback )
	{
		...
		timer && _top.clearTimeout(timer);
		callback && callback.call(that);
		
		if(second)
		{
			that._timer = _top.setTimeout(function(){
				that._click('cancel');
			}, 1000 * second);
		}
	...
	close: function()
	{
		var that = this, DOM = that.DOM,
			wrap = DOM.wrap,
			list = lhgdialog.list,
			fn = (that.config != null && !that.config.closed)?that.config.close:{};
		...
		if( that.iframe )
		{
			...
			if( that._frmTimer ) _top.clearTimeout(that._frmTimer);
		}
		else
		{
	...
	_iframe: function( url )
	{
		...
		that._frmTimer = _top.setTimeout(function(){
		    $iframe.attr('src', url);
		}, 1);
		...
		var load = that._fmLoad = function()
		{
			...
			// 适应iframe尺寸
			_top.setTimeout(function(){
			    iframe.style.cssText = loadCss;
			},0);// _top.setTimeout: 防止IE6~7对话框样式渲染异常
	...
	_addEvent: function()
	{
		...
		{
			resizeTimer && _top.clearTimeout(resizeTimer);
			resizeTimer = _top.setTimeout(function()
			{
				that._reset();
			}, 140);
		};
